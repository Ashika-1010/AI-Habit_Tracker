...
export const weeklyReport = async (req, res) => {
    try {
        ...
        if (!ctx.perHabit.length) {
            return res.json({
                content:
                    "You don't have any active habits yet. Create your first habit to start tracking and I'll generate a weekly report."
            });
        }
...
export const recoveryPlan = async (req, res) => {
    try {
        ...
        const userMsg =
            ...
            `Recent completions: ${logs.length}\n\n` +
        ...
        const context = habits
            .map((h) => {
                ...
                return `${h.name} (${h.category}): ${
                    hLogs.length
                }/30 in last 30 days, by weekday ${JSON.stringify(byDow)}`;
            })
            .join("\n");
